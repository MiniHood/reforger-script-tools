...
Callable symbols use `SymbolIndex::callable_signature()`, including source-backed parameter modifiers such as `out`, `inout`, and `notnull`. Non-callable symbols use existing indexed detail text such as type text, base type, enum value, default value, typedef aliased type, parameter detail, local-variable detail, or a bare preprocessor macro label. Documentation comments are preserved as raw copied comment text. `documentation_preview` is display-only: it strips comment markers and separator noise, trims common block-comment `*` prefixes, and lightly renders first-useful Doxygen-style tags such as `\brief`, `\param`, `\return`, `\warning`, and `\note` into readable preview text.
