...
The server handles `initialize`, `initialized`, `shutdown`, `exit`, full text-document sync notifications, `textDocument/documentSymbol`, `textDocument/hover`, and the custom debug request `reforger/debugHover`. Open documents are stored in memory by URI. Document symbols, hover results, and hover debug reports are built from the current document text through parser, AST, model, file-local index, `IndexQuery`, and `SymbolDisplay`. When the TypeScript client provides game-data paths, the server starts a background load/build of a cached game-data index and uses it as external resolver context after it becomes ready.
