...
Signature Help explains callable argument lists while the user is typing. The server advertises `(` and `,` trigger characters, reuses cached open-document analysis plus workspace/game-data external layers, and uses the same callable signature parser shared with completion. It supports ordinary functions, methods, member calls, constructors, `new Type(...)`, attributes such as `[Attribute(...)]` and `[RplRpc(...)]`, named arguments, nested calls, optional/defaulted parameters, enum-typed parameters, and Doxygen `\param` / `\return` documentation where indexed. Signature Help does not insert or format text; it only reports signatures, active argument index, parameter docs, required/optional/default facts, and callable docs. Unresolved call targets or positions outside callable argument lists return `null`.
