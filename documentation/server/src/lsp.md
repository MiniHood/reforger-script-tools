...
`reforger/debugCompletion` is the equivalent targeted troubleshooting request for autocomplete. It runs the same cached-analysis projection as `textDocument/completion` at the requested cursor position and returns a bounded Markdown report with completion context, receiver/owner inference, prefix replacement text, candidate counts, source/origin counts, phase timings, and the first completion items. It is intentionally command-triggered only; normal completion logging remains concise and does not serialize completion item tables.
