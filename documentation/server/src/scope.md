...
`LexicalScopeModel::from_parse_and_index` builds a root scope, callable scopes, and nested block scopes from the syntax tree. Parameters attach to callable scopes. Local variables attach to the innermost block scope containing their declaration. `visible_symbols_named` walks from the innermost scope outward and returns visible local/parameter candidates in shadowing order, with locals declared before the cursor beating later or outer declarations.
