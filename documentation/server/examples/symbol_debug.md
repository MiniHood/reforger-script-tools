...
The example requires `--file <path>` and accepts optional `--symbol <name>` or `--line <line>`. It parses the file, builds an `AstSourceFile`, creates a `SymbolCatalog`, and prints compact Markdown-style output.
