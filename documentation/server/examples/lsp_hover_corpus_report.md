...
The report uses the game-data index as external context. Remaining misses are unresolved after file-local and external top-level lookup. It still does not perform semantic receiver inference, inherited member lookup, Workbench validation, or workspace indexing.
