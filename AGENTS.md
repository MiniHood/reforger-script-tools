...
- Commit and push actions require explicit user authorization. A plan, workflow, route, or model assignment is not authorization. The sole exception is the project-owned verified auto-commit protocol: after the final focused check passes through `tools/verified-refactor-auto-commit.mjs verify --title <one-to-five-word-title> -- <check>`, the trusted Codex `Stop` hook may commit all working-tree changes on the exact `Refactor` branch. It must skip failed, missing, stale, mismatched, invalid, clean-tree, or Git-operation states. No automatic path may push, tag, alter remotes, or create, switch, merge, rebase, reset, delete, or rewrite branches/history. Review and trust the project hook through Codex's normal flow; never bypass hook trust.
