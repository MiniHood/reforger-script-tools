...
The existing `RSTIDX09` binary-v2 cache is a one-way compatibility input during the v11 rollout. It is accepted only when its exact v9 schema, shape, crate version, source fingerprint, file/symbol ownership, parent graph, and per-file counts all validate. The loader then reconstructs canonical public facts from those source-derived records, validates the current contribution contract, atomically replaces the bytes with `RSTIDX11`, and uses only the reconstructed current index. Wrong identity/version/shape or malformed old records rebuild from source; v9 is never retained as a runtime query path.
