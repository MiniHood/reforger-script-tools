...
- `server/src/lsp/callable.rs` owns reusable source-backed callable signature parsing and callable argument-list context helpers shared by completion and signature help.
...
- `server/src/lsp/external_overlay.rs` owns runtime game-data/workspace external layers, workspace aggregate rebuilds, update/delete handling, generation tracking, and status summaries.
...
- `server/src/lsp/signature_help.rs` owns source-backed `textDocument/signatureHelp` projection, active-parameter selection, signature documentation, and signature-help reports.

`lsp.rs` should keep request dispatch, runtime logging, and shared protocol helpers. Do not put substantial feature projection logic, open-document analysis ownership, or external layer indexing internals back into this file when a child module owns that feature.

## Current Behavior

The server handles `initialize`, `initialized`, `shutdown`, `exit`, full text-document sync notifications, `textDocument/documentSymbol`, `textDocument/hover`, `textDocument/definition`, `textDocument/completion`, `textDocument/signatureHelp`, `textDocument/semanticTokens/full`, live workspace overlay notifications, parser-diagnostic publishing, and the custom debug requests `reforger/debugHover` and `reforger/debugCompletion`. The initialize response advertises explicit text-document sync options with `openClose: true` and full-sync changes, rather than relying on the legacy numeric shorthand, so the TypeScript language client should send `didOpen`, full `didChange`, and `didClose` notifications for Enforce documents. Open documents are stored in memory by URI with the current full source text, optional LSP version, an internal revision counter, cached file-local analysis, and cached document-symbol projection. The cached analysis is rebuilt synchronously on `didOpen` and full-sync `didChange`, then reused by document symbols, hover, definition, completion, signature help, semantic tokens, parser diagnostics, and debug requests until the document changes again. Cached analysis includes parse, file-local index, lexical scope model, parser diagnostic count, and parser diagnostics. Document symbols are projected on `didOpen`, invalidated on `didChange`, and rebuilt lazily on the next `textDocument/documentSymbol` request so Outline projection does not block completion or hover after every keystroke in large files. Hover results, definition locations, completion items, signature-help results, semantic tokens, parser diagnostics, and debug reports are built from cached parser -> AST -> model -> file-local index -> lexical scope analysis through the resolver, `IndexQuery`, and `SymbolDisplay` where presentation is needed. Semantic tokens additionally keep a rich-token cache keyed by open-document revision and external generation. When the TypeScript client provides game-data paths, the server starts a background load/build of a cached game-data index. Debug and release builds both load game data when paths are configured, because development hover/definition must see the same external source set as packaged builds. When the TypeScript client provides workspace script roots, the server also builds a live workspace index. Resolver consumers query open-document facts first, then workspace external layer, then game-data external layer; no runtime request path consumes a pre-merged workspace+game-data `SymbolIndex`.

Hover converts LSP UTF-16 positions to byte offsets, asks `server/src/resolver.rs` for the hover target, and returns compact Markdown from `SymbolDisplayInfo`. The resolver owns both identifier resolution and syntax-span hover for non-identifier positions inside declaration spans, so LSP does not maintain a second hover selection path. File-local candidates still win over external candidates, and workspace external candidates win over game-data candidates. Once external layers are ready, unresolved type-like/top-level identifiers can resolve to workspace or game-data symbols such as mod classes, engine classes, and generated API declarations. Receiver/member-call hover uses resolver-owned AST expression views for simple receivers such as locals, parameters, fields, `this`, `super`, static type names, enum static members, known-return call receivers, `Type.Cast(...)`, and simple chains. Named argument labels are suppressed by the resolver so labels such as `level:` and `desc:` do not appear as unresolved hovers. The external game-data cache is runtime-pruned and intentionally omits local variables, but open-document analysis remains full and still supports local-variable hover in the active file. Workspace external indexes are live and uncached. If external layers are missing, building, failed, or unavailable, hover continues with file-local behavior. Hover still does not perform full expression typing, overload resolution, Workbench validation, or semantic `modded` merging.
...
Completion currently covers member access plus prefix-based type/top-level identifiers. The server advertises `"."` as the trigger character and also responds to manual completion requests on identifier prefixes such as `SCR_`, `array<SCR_`, and `GetG`. The resolver owns member-completion context detection, receiver type inference, static-owner detection, and top-level/type prefix context detection. Member completion uses the full receiver expression ending at the dot, so chained receivers such as `wrapper.m_Value.` complete against the final inferred owner rather than the last field name alone. Completion queries the open-document file-local index, workspace external layer, and game-data external layer separately, then combines the returned candidates with open-document candidates first. It must not build a full merged `SymbolIndex` per completion request. Instance member completion maps `IndexQuery::completion_members_for_class` candidates into completion items. Static-owner completion maps `IndexQuery::completion_static_members_for_type` candidates into completion items, so enum owners such as `LogLevel.` expose enum members and class owners expose copied static fields/methods plus the source-backed engine `Class.Cast` method when indexed. Direct `new Type(...)` receivers infer `Type` for member completion and hover. Typedef owner completion expands source-backed typedef targets such as `typedef array<int> TIntArray;` to the target owner where practical. Type completion returns class, enum, and typedef candidates. Top-level value/callable completion returns source-backed classes, enums, typedefs, functions, global fields, and enum members. Completion lists are capped at 250 items after ranking. Keyword completion is context split: statement/expression keywords stay available in value contexts, while declaration/modifier keywords only appear at clear declaration boundaries and do not affect source-backed symbol/type candidates. Completion items use `SymbolDisplay` labels, details/signatures, documentation previews, label details for callable parameter/return shapes, source-aware sort text, and text edits that replace only the typed prefix. Callable completions use snippet insert text for required parameters, omit optional/defaulted parameters from insertion, and default required enum-typed parameters to selected `EnumOwner.` placeholders when the owner is proven from indexed enum symbols. Enum-placeholder items attach an extension-owned trigger command so enum-member completion opens at the dot while the placeholder remains replaceable. It does not provide `completionItem/resolve`, full overload UI, diagnostics, fuzzy matching, semantic typedef/generic instantiation, or semantic `modded` merge behavior in this slice.

Signature Help explains callable argument lists while the user is typing. The server advertises `(`, `,`, `.`, `:`, `_`, letters, and digits as trigger/retrigger characters so parameter help can appear while users type or edit argument values, not only immediately after opening a call or typing a comma. The projection reuses cached open-document analysis plus workspace/game-data external layers and uses the same callable signature parser shared with completion. It supports ordinary functions, methods, member calls, constructors, `new Type(...)`, attributes such as `[Attribute(...)]` and `[RplRpc(...)]`, named arguments, nested calls, optional/defaulted parameters, enum-typed parameters, and Doxygen `\param` / `\return` documentation where indexed. Signature Help does not insert or format text; it only reports signatures, active argument index, parameter docs, required/optional/default facts, and callable docs. Unresolved call targets or positions outside callable argument lists return `null`.

Semantic tokens are the only Enforce coloring path. The server advertises a fixed semantic-token legend and computes full-document tokens from lexer facts plus cached index symbols. Lexer facts color comments, strings, numbers, keywords, operators, punctuation, preprocessor lines, and source-backed attribute syntax. Indexed symbol facts color declarations such as classes, enums, typedefs, functions, methods, fields, parameters, local variables, enum members, and type parameters, with modifiers such as declaration, static, readonly, and modification where available. To keep first paint responsive on large files, live semantic-token requests use a two-tier path: the first request for a document revision returns fast lexer/declaration/source-shape tokens, then the stdio server waits for a short idle delay before starting one resolver-backed rich projection for that revision on a worker thread. If the document revision or external overlay generation changes during the delay, the rich projection is skipped before paying resolver cost. Once a rich worker starts, the pending projection has a cancellation token that is set by `didChange` and external generation changes; the resolver-backed token loop checks this token so obsolete rich work can stop during projection instead of running for seconds and then being discarded. When a rich worker reports back, the server caches it and requests `workspace/semanticTokens/refresh`; the follow-up request returns the cached rich projection. The direct in-process `run` helper used by tests can still compute the rich projection synchronously because it does not represent the live extension request loop. The stdio runtime polls for external overlay generation changes while idle, so game-data/workspace index readiness can request semantic-token refreshes without waiting for a hover or other editor request. The old TextMate grammar path is intentionally removed so token coloring has one source of truth: Rust LSP semantic tokens consumed by the bundled semantic-token theme.
...
The server writes concise human-review logs to the optional `--log` path. Startup logs record server version, game-data path provenance, cache path, and external-index status. Background index logs record load/build start, cache status, file/symbol counts, parse diagnostics, and elapsed milliseconds. Foreground LSP request logs and background index logs share one mutex-backed logger so concurrent writes do not interleave or corrupt log lines. Live stdio request logs include `queue_ms`, the time from JSON-RPC message receipt on the reader thread until the main LSP loop started handling that message; this distinguishes slow execution from requests waiting behind earlier work. `didOpen` and `didChange` logs include URI, byte length, LSP version when available, internal revision, split analysis timings for parse/catalog/index/scope/build, document-symbol cache state, parse-diagnostic count, and queue wait; `didOpen` also includes immediate document-symbol projection time, while `didChange` marks document symbols as pending until the next document-symbol request. `documentSymbol`, `hover`, `definition`, `signatureHelp`, and `debugHover` logs include the cached document revision and mark that cached analysis was used; `documentSymbol` logs also mark whether cached document-symbol projection was reused or lazily rebuilt and how long projection took. Hover and definition logs include URI, byte length, hit/miss state, selected source, resolver reason, identifier context, resolver candidate count, external-index status, selected label/kind when available, parse-diagnostic count, queue wait, and elapsed milliseconds. Signature-help logs include callable context, active parameter, candidate count, selected callable label, failure reason, external-index status, parse-diagnostic count, phase timings, queue wait, and elapsed milliseconds. Semantic-token logs include projection mode (`fast-compute` or `rich-cache`), token counts, external-index status, phase timings, resolver call count, queue wait, and elapsed time; rich projection preparation logs separately when the cached projection is ready and a refresh is requested. Runtime logs must stay summary-only; they must not include full source text, full symbol trees, AST dumps, index dumps, or full hover Markdown.
...
`reforger/debugCompletion` is the equivalent targeted troubleshooting request for autocomplete and callable argument help. It runs the same cached-analysis projection as `textDocument/completion` and `textDocument/signatureHelp` at the requested cursor position, then returns one bounded Markdown report with completion context, receiver/owner inference, prefix replacement text, candidate counts, source/origin counts, completion phase timings, first completion items, signature-help context, active parameter, signature candidates, parameter docs, and signature-help timings. It is intentionally command-triggered only; normal completion and signature-help logging remains concise and does not serialize completion item tables or signature tables.
...
`server/examples/lsp_signature_help_report.rs` provides a dev-only fixture report for signature help. It writes `tools/reports/lsp-signature-help-fixtures.report.md` with targeted checks for functions, methods, constructors, `new Type(...)`, attributes, named arguments, enum-typed parameters, optional parameters, and non-call misses.
...
- Switched initialize capabilities to explicit full text-document sync options with `openClose: true`.
...
- Moved live stdio rich semantic-token projection onto worker threads. The LSP request loop returns the fast response, then applies the rich cache only when the worker result still matches the open-document revision and external-overlay generation.
- Added an idle-delay gate before starting live rich semantic-token workers so rapid edits in large files do not keep computing resolver-backed colors for stale revisions.
- Added cancellation tokens to live rich semantic-token workers so rapid edits can stop obsolete resolver-backed coloring during the expensive projection path.
...
- Added live request `queue_ms` logging so autocomplete loading can be diagnosed as execution time versus waiting behind earlier LSP work.
...
- Made document-symbol projection lazy after edits and added split open-document analysis timings so large-file autocomplete troubleshooting can distinguish parser/index/scope work from Outline projection work.
...
- Added source-backed `textDocument/signatureHelp` support and the dev-only LSP signature-help fixture report.
- Extended `reforger/debugCompletion` so the existing Ctrl+F2 troubleshooting path includes Signature Help context instead of adding another user-facing debug command.
