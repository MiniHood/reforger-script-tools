...
`OpenDocument::new` builds `FileIndexAnalysis` from full source text and records split analysis timings for parse, catalog, index, and scope construction. `lsp.rs` supplies each URI with a persistent monotonic revision when it creates an `OpenDocument`; `OpenDocument::replace` increments that revision while updating the full text and optional version, rebuilding analysis synchronously, clearing the document-symbol projection, and clearing revision-bound semantic-token cache data. `file_index_for_source` is still re-exported from `lsp.rs` for reports and tests. `FileIndexAnalysis` contains the parse tree, cached lexer tokens, file-local index, lexical scope model, parse diagnostic count, and cloned diagnostics. The tokens are created in the same full-sync replacement transaction and reused by live completion and semantic-token projection.
