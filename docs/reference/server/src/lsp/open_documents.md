...
This child module sits under `server/src/lsp.rs`. It keeps document text, required LSP version, internal revision, and cached parser/AST/model/index/scope analysis together so request dispatch can reuse one authoritative open-file analysis across document symbols, hover, definition, completion, diagnostics, semantic tokens, and debug-hover.

## Current Behavior

`OpenDocument::new` builds `FileIndexAnalysis` from full source text and records split analysis timings for parse, catalog, index, and scope construction. `lsp.rs` supplies each URI with a persistent monotonic revision when it creates an `OpenDocument`; `OpenDocument::replace` increments that revision while updating the full text and required version, rebuilding analysis synchronously, clearing the document-symbol projection, and clearing revision-bound semantic-token cache data. Dispatch only calls `replace` for a strictly newer full-sync `didChange` version on an already-open URI, so stale replayed changes cannot replace cached analysis. `file_index_for_source` is still re-exported from `lsp.rs` for reports and tests. `FileIndexAnalysis` contains the parse tree, cached lexer tokens, file-local index, lexical scope model, parse diagnostic count, and cloned diagnostics. The tokens are created in the same full-sync replacement transaction and reused by live completion and semantic-token projection.
