...
Live workspace mutations are latest-wins. The LSP client supplies a monotonic sequence per absolute, lexically normalized path identity; the overlay retains the last accepted sequence even for deletion tombstones and rejects equal or older events before parsing or publishing an aggregate. Filesystem canonicalization remains an indexing-path concern and is never used for this sequence identity, because a deleted path may no longer resolve. Startup scanning classifies each `DirEntry` before recursing: symbolic-link directories are skipped on every platform, and Windows directory reparse points (including junctions) are skipped as well. This keeps discovery inside physical configured roots rather than following linked trees.
