...
Until the erased-prefix Suggest lifecycle is proven, Ctrl+F2 also appends a temporary, in-memory completion-lifecycle trace for the active document. The ring holds at most 80 events and records only document version/change direction, LSP completion request and response versions, trigger kind, item count, `isIncomplete`, and recovery-dispatch state. It never records source text, cursor text, URIs, ranges, candidates, or completion payloads. OpenSpec change `fix-rplrpc-enum-completion-contract` task 3.4 requires removing this forensic trace before release.
