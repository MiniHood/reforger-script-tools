...
Formatting is currently planning-only. The temporary formatter architecture note under `docs/reference/server/src/formatting.md` defines the intended Rust-side boundary for future document formatting, on-type formatting, typing assists, attribute assists, comment/Doxygen formatting, and formatting reports.
