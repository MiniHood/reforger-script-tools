...
| [`search_game_data_text`](mcp-api/tools/search_game_data_text.md) | `cursor?`, `limit?`, `matchCase?`, `matchWholeWord?`, `query`, `useRegex?` | `catalogueRevision`, `nextCursor?`, `query`, `results`, `returned`, `stats`, `total`, `truncated` | Explicitly scan readable Game Data source text for literal matches. |
