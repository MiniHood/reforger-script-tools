...
const WORKBENCH_RESTART_DESCRIPTION: &str = "Gracefully close one exact observed Workbench process and relaunch its one resolved Enfusion Workbench project with -gproj only after the original exits. It refuses to close Workbench if the visible project window or exact local project descriptor cannot be resolved.";
