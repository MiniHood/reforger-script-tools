...
struct BuildTimings {
    file_discovery: Duration,
    catalog_build: Duration,
...
fn render_report(scripts_path: &Path) -> Result<String, String> {
    ...
    files.sort();
    let file_discovery = file_discovery_start.elapsed();
    ...
    let catalog_build = catalog_build_start.elapsed();

    let index_build_start = Instant::now();
    let index = SymbolIndex::from_catalogs(catalogs.iter());
    let index_build = index_build_start.elapsed();
    ...
    append_build_timing(&mut report, &timings);
...
fn append_build_timing(report: &mut String, timings: &BuildTimings) {
    ...
    report.push_str(&format!(
        "| Report rendering | {} |\n",
        duration_millis(timings.report_render)
    ));
    report.push_str(&format!(
        "| Total report run | {} |\n\n",
...
fn method_signatures(index: &SymbolIndex, ids: &[GlobalSymbolId]) -> Vec<String> {
    let mut signatures = ids
        ...
        .filter_map(|id| index.method_signature(*id))
        .collect::<Vec<_>>();
