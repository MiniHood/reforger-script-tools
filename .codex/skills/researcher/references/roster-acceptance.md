...
| Narrow relevance | A question with only two material local lenses | Coordinator records considered specialists and runs only those two. |
...
| Evidence snapshot | A repository changes during a research run | Final brief records its base revision and labels later evidence as post-snapshot rather than merging states. |
| Durable option | Evidence supports one complete durable design and one temporary mitigation | Durable target ranks first; mitigation states limitation and removal condition. |
| One viable option | Evidence rejects all alternatives | Brief presents one viable option and explains why alternatives were unsupported. |
